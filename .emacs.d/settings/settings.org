...
(define-key my-mode-map (kbd "C-x k") 'bjm/kill-this-buffer)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defhydra my/select-themes (:color blue
                                     :hint nil)
    "
    Themes:
    ^^^^-----------------------------------------------------------------------
    _z_: zenburn             _e_: deeper-blue
    _m_: monokai             _w_: wheatgrass
    _d_: solarize-dark       _r_: dark-laptop
    _s_: solarized-light
      "

    ("z" (load-theme 'zenburn t))
    ("m" (load-theme 'monokai t))
    ("d" (load-theme 'solarized-dark t))
    ("s" (load-theme 'solarized-light t))

    ("e" (load-theme 'deeper-blue t))
    ("w" (load-theme 'wheatgrass t))
    ("r" (load-theme 'dark-laptop t))

    ("q" nil "quit" :color blue))

  (global-set-key (kbd "<f7>") 'my/select-themes/body)
#+END_SRC
      ...
      (progn
        ...
        (setq elfeed-feeds
              '(("https://radio-t.com/podcast.rss" podcast)
                ...
                ("https://www.reddit.com/r/emacs/.rss" blog)
                ("https://habrahabr.ru/rss/flows/develop/all/" blog)
                ;;; электроника
                ("http://embedded.fm/?format=rss" podcast electronix)
                ("http://embedded.fm/blog/?format=rss" blog electronix)
                ("https://marsohod.org/component/ninjarsssyndicator/?feed_id=1&format=raw" blog electronix)
                ("https://hackaday.com/blog/feed/" blog electronix)
                ;;; youtube channels
                ;; adafruit industries
...
** Highlight TODO
   [[https://github.com/tarsius/hl-todo][src]]
#+BEGIN_SRC emacs-lisp
  (use-package hl-todo
    :ensure t
...
#+BEGIN_SRC emacs-lisp
  (use-package verilog-mode
    ...
    (progn
      ...
      (add-hook 'verilog-mode-hook 'highlight-indentation-mode)
...
#+BEGIN_SRC emacs-lisp
    (use-package server
      ...
      (progn
        ...
        ;; https://www.reddit.com/r/emacs/comments/1s9tfk/emacs_server_mode_on_windows/
        ;; https://www.emacswiki.org/emacs/EmacsAsDaemon

        ;; запускаем клиента:
        ;; emacsclient -c -n --server-file ~/.emacs.d/server/emacs-server-file

        ;; чтобы завершить работу сервера, вызываем:
        ;;
        ;; emacsclient --server-file ~/.emacs.d/server/emacs-server-file -e
        ;; '(kill-emacs)'
